...
df=pd.read_excel(r'C:\Users\szu\Desktop\规土委_区域部分\粤港澳网格点_02deg.xlsx',sheet_name=0)
df['temp']=df.apply(lambda row: gcj02_to_wgs84(row['centerX'], row['centerY']), axis=1)
df['centerX_gcj']=df['temp'].map(lambda x:round(x[0],6))
df['centerY_gcj']=df['temp'].map(lambda x:round(x[1],6))
df=df.drop('temp',axis=1)
df.to_excel(r'C:\Users\szu\Desktop\规土委_区域部分\粤港澳网格点_02deg_gcj.xlsx',index=False,encoding='utf8')
